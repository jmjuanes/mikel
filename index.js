...
const parseArgs = (str = "", data = {}, globals = {}, fns = {}, argv = [], opt = {}) => {
    const [t, ...args] = tokenizeArgs(str.trim());
    args.forEach(argStr => {
        if (argStr.includes("=") && !argStr.startsWith(`"`)) {
            const [k, v] = argStr.split("=");
            opt[k] = parse(v, data, globals, fns);
        }
        else if (argStr.startsWith("...")) {
            const value = parse(argStr.replace(/^\.{3}/, ""), data, globals, fns);
            if (!!value && typeof value === "object") {
                Array.isArray(value) ? argv.push(...value) : Object.assign(opt, value);
            }
        }
        else {
            argv.push(parse(argStr, data, globals, fns));
        }
    });
    return [t, argv, opt];
};
...
const evaluateExpression = (str = "", data = {}, globals = {}, fns = {}) => {
    const [ fnName, args, opt ] = parseArgs(str, data, globals, fns);
    if (typeof fns[fnName] === "function") {
        return fns[fnName]({args, opt, options: opt, data, variables: globals});
    }
...
const parse = (v, data = {}, globals = {}, fns = {}) => {
    if (v.startsWith("(") && v.endsWith(")")) {
        return evaluateExpression(v.slice(1, -1).trim(), data, globals, fns);
    }
    if ((v.startsWith(`"`) && v.endsWith(`"`)) || /^-?\d+\.?\d*$/.test(v) || v === "true" || v === "false" || v === "null") {
        return JSON.parse(v);
    }
    return (v || "").startsWith("@") ? get(globals, v.slice(1)) : get(data, v || ".");
};
...
const create = (options = {}) => {
    ...
    const compile = (tokens, output, data, globals, index = 0, section = "") => {
        let i = index;
        while (i < tokens.length) {
            if (i % 2 === 0) {
                ...
                const [t, args, opt] = parseArgs(tokens[i].slice(1), data, globals);
                const j = i + 1;
                ...
                output.push(ctx.helpers[t]({
                    ...
                    data: data,
                    variables: globals,
                    fn: (blockData = {}, blockGlobals = {}, blockOutput = []) => {
                        const helperGlobals = {
                            ...globals,
                            ...blockGlobals,
                            helper: {
                        ...
                        compile(tokens, blockOutput, blockData, helperGlobals, j, t);
                ...
                if (!negate && value && Array.isArray(value)) {
                    ...
                    (value.length > 0 ? value : [""]).forEach(item => {
                        i = compile(tokens, value.length > 0 ? output : [], item, globals, j, t);
                    });
                }
                else {
                    const includeOutput = (!negate && !!value) || (negate && !!!value);
                    i = compile(tokens, includeOutput ? output : [], data, globals, i + 1, t);
                }
            }
            // DEPRECATED
                ...
                const [t, args, opt] = parseArgs(tokens[i].replace(/^>{1,2}/, ""), data, globals);
                const blockContent = []; // to store partial block content
                const partials = Object.assign({}, ctx.partials, globals.partials || {});
                if (tokens[i].startsWith(">>")) {
                    i = compile(tokens, blockContent, data, globals, i + 1, t);
                }
                if (typeof partials[t] === "string" || typeof partials[t]?.body === "string") {
                    ...
                    compile(tokenize(partials[t]?.body || partials[t]), output, partialData, partialGlobals, 0, "");
                }
            }
            else if (tokens[i].startsWith("=")) {
                output.push(evaluateExpression(tokens[i].slice(1), data, globals, ctx.functions) ?? "");
            }
            else if (tokens[i].startsWith("/")) {
                ...
                const t = tokens[i].split("||").map(v => {
                    ...
                    if (v.trim().startsWith("!")) {
                        return parse(v.trim().slice(1).trim(), data, globals);
                    }
                    // escape the returned value
                    return escape(parse(v.trim(), data, globals));
                });
